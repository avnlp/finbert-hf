...
"""
This script converts a lm-head checkpoint from the "Token Dropping" implementation into a PyTorch-compatible FINBERT
...
from transformers import FinBERTConfig, FinBERTForMaskedLM
from transformers.models.finbert.modeling_finbert import (
    FinBERTIntermediate,
    FinBERTLayer,
    FinBERTOutput,
    FinBERTPooler,
...
def convert_checkpoint_to_pytorch(tf_checkpoint_path: str, config_path: str, pytorch_dump_path: str):
    ...
    config = FinBERTConfig.from_json_file(config_path)
    model = FinBERTForMaskedLM(config)

    # Layers
    for layer_index in range(0, config.num_hidden_layers):
        layer: FinBERTLayer = model.finbert.encoder.layer[layer_index]

        # Self-attention
        self_attn: FinBERTSelfAttention = layer.attention.self
        ...
        self_output: FinBERTSelfOutput = layer.attention.output
        ...
        # Intermediate
        intermediate: FinBERTIntermediate = layer.intermediate
        ...
        # Output
        finbert_output: FinBERTOutput = layer.output

        finbert_output.dense.weight.data = get_encoder_layer_array(layer_index, "_output_dense/kernel")
        finbert_output.dense.bias.data = get_encoder_layer_array(layer_index, "_output_dense/bias")

        finbert_output.LayerNorm.weight.data = get_encoder_layer_array(layer_index, "_output_layer_norm/gamma")
        finbert_output.LayerNorm.bias.data = get_encoder_layer_array(layer_index, "_output_layer_norm/beta")

    # Embeddings
    model.finbert.embeddings.position_embeddings.weight.data = get_encoder_array("_position_embedding_layer/embeddings")
    model.finbert.embeddings.token_type_embeddings.weight.data = get_encoder_array("_type_embedding_layer/embeddings")
    model.finbert.embeddings.LayerNorm.weight.data = get_encoder_array("_embedding_norm_layer/gamma")
    model.finbert.embeddings.LayerNorm.bias.data = get_encoder_array("_embedding_norm_layer/beta")
    ...
    model.finbert.embeddings.word_embeddings.weight.data = get_masked_lm_array("embedding_table")

    # Pooling
    model.finbert.pooler = FinBERTPooler(config=config)
    model.finbert.pooler.dense.weight.data: FinBERTPooler = get_encoder_array("_pooler_layer/kernel")
    model.finbert.pooler.dense.bias.data: FinBERTPooler = get_encoder_array("_pooler_layer/bias")

    # Export final model
    model.save_pretrained(pytorch_dump_path)

    # Integration test - should load without any errors ;)
    new_model = FinBERTForMaskedLM.from_pretrained(pytorch_dump_path)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="The config json file corresponding to the FINBERT model. This specifies the model architecture.",
    )
    parser.add_argument(
        "--pytorch_dump_path",
        type=str,
    ...
    convert_checkpoint_to_pytorch(args.tf_checkpoint_path, args.finbert_config_file, args.pytorch_dump_path)
