...
from transformers import FinBERTConfig, is_tf_available
...
if is_tf_available():
    ...
    from transformers.models.finbert.modeling_tf_finbert import (
        ...
        TFFinBERTLMHeadModel,
        TFFinBERTModel,
    )


class TFFinBERTModelTester:
    def __init__(
        self,
        parent,
        batch_size=13,
    ...
    def create_and_check_model(
        ...
        model = TFFinBERTModel(config=config)
    ...
    def create_and_check_causal_lm_model(
        ...
        model = TFFinBERTLMHeadModel(config=config)
        inputs = {
            "input_ids": input_ids,
            "attention_mask": input_mask,
            "token_type_ids": token_type_ids,
    ...
    def create_and_check_for_masked_lm(
        ...
        model = TFFinBERTForMaskedLM(config=config)
        inputs = {
            "input_ids": input_ids,
            "attention_mask": input_mask,
            "token_type_ids": token_type_ids,
    ...
    def create_and_check_for_next_sequence_prediction(
        ...
        model = TFFinBERTForNextSentencePrediction(config=config)
        inputs = {"input_ids": input_ids, "attention_mask": input_mask, "token_type_ids": token_type_ids}
        result = model(inputs)
    ...
    def create_and_check_for_pretraining(
        ...
        model = TFFinBERTForPreTraining(config=config)
        inputs = {"input_ids": input_ids, "attention_mask": input_mask, "token_type_ids": token_type_ids}
        result = model(inputs)
    ...
    def create_and_check_for_sequence_classification(
        ...
        model = TFFinBERTForSequenceClassification(config=config)
        inputs = {
            "input_ids": input_ids,
            "attention_mask": input_mask,
            "token_type_ids": token_type_ids,
    ...
    def create_and_check_for_multiple_choice(
        ...
        model = TFFinBERTForMultipleChoice(config=config)
    ...
    def create_and_check_for_token_classification(
        ...
        config.num_labels = self.num_labels
        model = TFFinBERTForTokenClassification(config=config)
        inputs = {
            "input_ids": input_ids,
            "attention_mask": input_mask,
            "token_type_ids": token_type_ids,
    ...
    def create_and_check_for_question_answering(
        ...
        model = TFFinBERTForQuestionAnswering(config=config)
        inputs = {
            "input_ids": input_ids,
            "attention_mask": input_mask,
            "token_type_ids": token_type_ids,
...
@require_tf
class TFFinBERTModelTest(TFModelTesterMixin, TFCoreModelTesterMixin, unittest.TestCase):
    ...
    def setUp(self):
        self.model_tester = TFFinBERTModelTester(self)
    ...
    def test_model_from_pretrained(self):
        model = TFFinBERTModel.from_pretrained("finbert")
        self.assertIsNotNone(model)

    def test_model_common_attributes(self):
        config, inputs_dict = self.model_tester.prepare_config_and_inputs_for_common()
        list_lm_models = [TFFinBERTForMaskedLM, TFFinBERTForPreTraining, TFFinBERTLMHeadModel]
    ...
    def test_custom_load_tf_weights(self):
        model, output_loading_info = TFFinBERTForTokenClassification.from_pretrained(
            "finbert", output_loading_info=True
        )
...
@require_tf
class TFFinBERTModelIntegrationTest(unittest.TestCase):
    @slow
    def test_inference_masked_lm(self):
        model = TFFinBERTForPreTraining.from_pretrained("lysandre/tiny-finbert-random")
